...
namespace winrt::HotelManagement::implementation
{
    ...
    Windows::Foundation::IAsyncAction Account::Page_Loaded(IInspectable const& sender, RoutedEventArgs const& e)
    {
        ...
        db << "SELECT paymentName FROM linked_eWallets WHERE username = ?;"
            ...
            auto btn_grd = Grid();
            auto cldef1 = ColumnDefinition(); cldef1.Width(GridLengthHelper::FromPixels(175));
            ...
            btn_del.Click([] (winrt::Windows::Foundation::IInspectable const& sender_del, winrt::Windows::UI::Xaml::RoutedEventArgs const& e_del) -> winrt::fire_and_forget {
                ...
                if (res == ContentDialogResult::Primary)
                {
                    ...
                    co_await Dialog(
                        L"Payment Method Removed",
                        paymentName_ev + L" account was successfully removed to your account!",
                        L"Ok");
                }
            });
        ...
        db << "SELECT paymentName FROM linked_Cards WHERE username = ?;"
            ...
            btn_del.Click([](winrt::Windows::Foundation::IInspectable const& sender_del, winrt::Windows::UI::Xaml::RoutedEventArgs const& e_del) -> winrt::fire_and_forget {
                ...
                auto res = co_await Dialog(
                    L"Removing the payment method account",
                    L"You are about remove your " + paymentName_ev + L" card from the list of your payment methods.\nWould you like to continue removing this account?",
                    L"No",
                    L"Yes");
                ...
                if (res == ContentDialogResult::Primary)
                {
                    ...
                    db << "DELETE FROM linked_Cards WHERE username = ? AND paymentName = ?;"
                        << userLogged
                        << to_string(paymentName_ev);

                    co_await Dialog(
                        L"Payment Method Removed",
                        paymentName_ev + L" card was successfully removed to your account!",
                        L"Ok");
                }
                });
